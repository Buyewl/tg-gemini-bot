    ...
    if update.type == "command":
        ...
        if update.is_group:
            log = (
                ...
                f"{the_content_sent_is}\n{update.text}\n"
                f"{the_reply_content_is}\n{response_text}\n"
                f"{the_logarithm_of_historical_conversations_is}{dialogueLogarithm}")
        else:
            log = (
                f"@{update.user_name} id:`{update.from_id}`"
                f"{the_content_sent_is}\n{update.text}\n"
        ...
        if update.is_group:
            log = (
                f"@{update.user_name} id:`{update.from_id}` {group} "
                f"@{update.group_name} id:`{update.chat_id}`[photo]({photo_url}),"
                f"{the_accompanying_message_is}\n{update.photo_caption}\n"
                f"{the_reply_content_is}\n{response_text}")
        else:
            log = (
                f"@{update.user_name} id:`{update.from_id}`[photo]({photo_url}),"
                f"{the_accompanying_message_is}\n{update.photo_caption}\n"
                f"{the_reply_content_is}\n{response_text}")
